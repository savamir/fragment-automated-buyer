...
async def purchase_lock():
    if app_state.buy_in_progress:
        raise HTTPException(
            ...
            detail="Purchase already in progress"
        )
...
class MonitorStartRequest(BaseModel):
    ...
    interval_sec: int = Field(default=1, ge=1, le=60, description="Monitoring interval in seconds")
...
async def validate_affordability(max_price: int, balance_ton: float, item_type: str = "items") -> None:
...
async def validate_balance_for_monitoring(max_price_ton: int, balance_ton: float, item_type: str = "items") -> None:
...
async def pre_check_affordability(max_price_ton: int, item_type: str = "items") -> float:
    try:
        ton = await get_ton_instance()
        balance_nano = await ton.get_balance_nano()
        balance_ton = balance_nano / 1_000_000_000
        
        logger.info(f"Pre-checking {item_type} affordability for max price: {max_price_ton} TON")
        ...
        client = get_fragment_client() if item_type == "items" else get_fragment_usernames_client()
        listings = await client.list_sales()
        
        affordable_items = [
            item for item in listings 
            if item.get("price_ton_int") is not None and item.get("price_ton_int") <= max_price_ton
        ]
        
        if not affordable_items:
            logger.info(f"No {item_type} found within price range (max: {max_price_ton} TON), but will start monitoring anyway")
            return balance_ton
        
        affordable_items.sort(key=lambda x: x.get("price_ton_int", float('inf')))
        cheapest_price = affordable_items[0].get("price_ton_int")
        
        logger.info(f"Found {len(affordable_items)} {item_type} within price range. Cheapest: {cheapest_price} TON")
        
        await validate_affordability(cheapest_price, balance_ton, item_type)
        
        logger.info(f"✅ {item_type.capitalize()} pre-check passed: Can afford {item_type} up to {max_price_ton} TON")
        return balance_ton
    ...
    try:
        ...
        async def process_listings():
            try:
                ...
                affordable_items = [
                    item for item in listings 
                    if item.get("price_ton_int") is not None and item.get("price_ton_int") <= body.max_price_ton
                ]
                ...
                try:
                    ...
                    if current_balance_ton < price:
                        logger.info(f"💰 Insufficient balance for {price} TON item, stopping monitor")
                ...
                logger.info(f"🎯 FOUND NUMBER TARGET: {cheapest_item.get('id')} for {price} TON")
                
                async with purchase_lock():
                    try:
                        buy_request = BuyRequest(
                            number_id=cheapest_item.get("id"),
                            bid_ton=price
                        )
                        
                        result = await asyncio.wait_for(buy_number(buy_request), timeout=60.0)
                        ...
                        try:
                            ...
                            logger.info(f"💰 Remaining balance: {remaining_ton:.2f} TON")
                            
                            if remaining_ton < 1.0:
                                logger.info("💰 Low balance, stopping number monitor")
                                app_state.monitor_should_stop = True
                            else:
                                logger.info("🔄 Sufficient balance remaining, will continue monitoring numbers...")
                        ...
                        logger.error("❌ NUMBER PURCHASE TIMEOUT: Function took too long")
                    except HTTPException as e:
                        logger.error(f"❌ NUMBER PURCHASE FAILED (HTTP {e.status_code}): {e.detail}")
                        if e.status_code == 402:
                            logger.info("💰 Stopping number monitor due to insufficient balance")
                            app_state.monitor_should_stop = True
                        else:
                            logger.info("🔄 Number purchase failed, will try again in next cycle")
                    except Exception as e:
                        logger.error(f"❌ NUMBER PURCHASE ERROR: {e}")
                        logger.info("🔄 Number purchase error, will try again in next cycle")
...
async def start_username_monitor(body: UsernameMonitorStartRequest) -> Dict[str, Any]:
    try:
        ...
        async def process_username_listings():
            try:
                ...
                try:
                    ...
                    if current_balance_ton < price:
                        logger.info(f"💰 Insufficient balance for {price} TON username, stopping monitor")
                        app_state.monitor_should_stop = True
                        return
                ...
                logger.info(f"🎯 FOUND USERNAME TARGET: {cheapest_item.get('username')} ({cheapest_item.get('id')}) for {price} TON")
                
                async with purchase_lock():
                    try:
                        buy_request = BuyUsernameRequest(
                            username_id=cheapest_item.get("id"),
                            bid_ton=price
                        )
                        
                        result = await asyncio.wait_for(buy_username(buy_request), timeout=60.0)
                        ...
                        try:
                            ...
                            if remaining_ton < 1.0:
                                ...
                                logger.info("🔄 Sufficient balance remaining, will continue monitoring usernames...")
                                
                        except Exception as e:
                            logger.warning(f"Could not check remaining balance: {e}")
                            logger.info("🔄 Will continue monitoring usernames anyway...")
                        
                    except asyncio.TimeoutError:
                        logger.error("❌ USERNAME PURCHASE TIMEOUT: Function took too long")
                    except HTTPException as e:
                        logger.error(f"❌ USERNAME PURCHASE FAILED (HTTP {e.status_code}): {e.detail}")
                        if e.status_code == 402:
                            logger.info("💰 Stopping username monitor due to insufficient balance")
                            app_state.monitor_should_stop = True
                        else:
                            logger.info("🔄 Username purchase failed, will try again in next cycle")
                    except Exception as e:
                        logger.error(f"❌ USERNAME PURCHASE ERROR: {e}")
                        logger.info("🔄 Username purchase error, will try again in next cycle")
        ...
        app_state.monitor = NumbersMonitor(
            ...
            on_new_listing=lambda item: asyncio.create_task(process_username_listings()),
            interval_sec=body.interval_sec,
        )
        await app_state.monitor.start()
...
async def buy_number(body: BuyRequest) -> Dict[str, Any]:
    logger.info(f"🔧 BUY FUNCTION START: number_id={body.number_id}, bid_ton={body.bid_ton}")
    ...
    try:
        ...
        info = await asyncio.wait_for(client.get_number_info(body.number_id), timeout=10.0)
        logger.info("🔧 Getting bid link...")
        prep = await asyncio.wait_for(client.api_get_bid_link(
        ...
        logger.info(f"🔧 BUY PREP: to={prep.get('address')} amount_nano={prep.get('amount_nano')}")
        logger.info("🔧 Attempting transfer...")
        
        res = await asyncio.wait_for(ton.transfer(
            ...
            if error_type == "insufficient_balance":
                raise HTTPException(
                    ...
                    detail={
                        ...
                        "shortage": (res.get('amount_nano', 0) - res.get('balance_nano', 0)) / 1_000_000_000,
                        "item_type": "number",
...
async def buy_username(body: BuyUsernameRequest) -> Dict[str, Any]:
    logger.info(f"🔧 BUY USERNAME FUNCTION START: username_id={body.username_id}, bid_ton={body.bid_ton}")
    ...
    try:
        ...
        info = await asyncio.wait_for(client.get_username_info(body.username_id), timeout=10.0)
        logger.info("🔧 Getting bid link...")
        prep = await asyncio.wait_for(client.api_get_bid_link(
