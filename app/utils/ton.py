...
from pytoniq import LiteClient, WalletV4R2, Cell, Address
...
class TON:
    ...
    async def _ensure(self) -> None:
        if self._client is None:
            try:
                self._client = LiteClient.from_mainnet_config(ls_i=5, trust_level=2, timeout=20)
    ...
    async def transfer(self, to_bounceable: str, amount_nano: int, payload_b64: str = "") -> Dict[str, Any]:
        try:
            await self._ensure()
            ...
            try:
                ...
                if bal < amount_nano:
                    logger.warning(f"Insufficient balance: need {amount_nano}, have {bal}")
                    return {
                        "ok": False, 
            ...
            for attempt in range(3):
                try:
                    ...
                    res = await wallet.raw_transfer(msgs=[msg], seqno_from_get_meth=True)
                    
                    if res and str(res) != "None":
                        logger.info(f"Transfer successful: {res}")
                        return {"ok": True, "amount_nano": amount_nano, "to": to_bounceable, "result": str(res)}
                    else:
                        logger.warning(f"Transfer failed: {res}")
                        return {"ok": False, "error": "transfer_failed", "amount_nano": amount_nano, "to": to_bounceable, "result": str(res)}
                ...
                try:
                    ...
                    return {"ok": True, "amount_nano": amount_nano, "to": to_bounceable, "result": "external_message_sent"}
                except Exception as e:
                    logger.error(f"External transfer failed: {e}")
                    return {"ok": False, "error": "external_transfer_failed", "amount_nano": amount_nano, "to": to_bounceable, "details": str(e)}
            
        except Exception as e:
            logger.error(f"Transfer operation failed: {e}")
            return {"ok": False, "error": "all_transfer_attempts_failed", "amount_nano": amount_nano, "to": to_bounceable, "details": str(e)}

    async def get_address(self) -> str:
        try:
            await self._ensure()
            return self._wallet.address.to_str(is_user_friendly=True, is_bounceable=False)
        except Exception as e:
    ...
    async def get_fragment_account_payload(self, chain: str = "-239") -> Dict[str, str]:
        try:
            ...
            state_init_boc_b64 = base64.b64encode(wallet.state_init.serialize().to_boc()).decode()
